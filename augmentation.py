...
class RandomVideoRotation(object):
    """Rotate entire clip randomly by a random angle within
    given bounds
    Args:
    degrees (sequence or int): Range of degrees to select from
    ...
    def __call__(self, clip):
        ...
        """
        angle = random.uniform(self.degrees[0], self.degrees[1])
        if isinstance(clip['video'][0], np.ndarray):
            rotated = [rotate(image=img, angle=angle, preserve_range=True) for img in clip['video']]
        elif isinstance(clip['video'][0], PIL.Image.Image):
            rotated = [img.rotate(angle) for img in clip['video']]
        else:
            raise TypeError('Expected numpy.ndarray or PIL.Image' +
                            'but got list of {0}'.format(type(clip['video'][0])))
        clip['video'] = np.array(rotated)
        return clip
...
class VideoColorJitter(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip['video'][0], np.ndarray):
            ...
            raise TypeError('Expected numpy.ndarray or PIL.Image' +
                            'but got list of {0}'.format(type(clip[0])))
        clip['video'] = np.array(jittered_clip)
        return clip
...
class SelectNonOverlappingFrames(object):
    def __init__(self, number_of_frames=16, skip_frame_count=2):
        self.number_of_frames = number_of_frames
        self.skip_frame_count = skip_frame_count

    def __call__(self, clip):
        result = clip[np.hstack((np.array(list(range(0, self.skip_frame_count*self.number_of_frames,
                                 self.skip_frame_count))),
                      np.array(list(range(-self.skip_frame_count*self.number_of_frames, 0,
                                 self.skip_frame_count)))))]
        return result
...
class SplitSourceTestDriving(object):
    def __call__(self, clip, number_of_frames=16):
        source = np.array(clip[number_of_frames], dtype='float32')
        video = np.array(clip[0:number_of_frames], dtype='float32')
        test = np.array(clip[number_of_frames:], dtype='float32')
        return {
            'test': test,
            'image': source,
            'video': video
...
class VideoDictToTensor(object):
    """Convert video array to Tensor."""

    def __call__(self, driving):
        driving['video'] = driving['video'].transpose(3,0,1,2)
        driving['image'] = driving['image'].transpose(2,0,1)
        driving['test'] = driving['test'].transpose(3,0,1,2)
        return driving
...
class ContentMotionTestTransform:
    def __init__(self, resize_param=None, rotation_param=None, flip_param=None, crop_param=None, jitter_param=None):
        self.transforms = []
        self.select = SelectNonOverlappingFrames()
        self.transforms.append(self.select)
        self.transforms.append(SplitSourceTestDriving())

        if rotation_param is not None:
            self.transforms.append(RandomVideoRotation(**rotation_param))

        if jitter_param is not None:
            self.transforms.append(VideoColorJitter(**jitter_param))

        self.transforms.append(VideoDictToTensor())
